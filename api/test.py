...
class HotelsViewTest(TestCase):
    ...
    def test_post_hotels_area_api(self):
        ...
        serializer = HotelsSerializer(hotels, many=True)
        self.assertEqual(response.data["hotels in range 10km"], serializer.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class BookingViewTest(TestCase):
    ...
    def test_get_booking(self):
        user = self.client.post(
            "/auth/jwt/create",
            data={"username": "test_api_user", "password": "QazxsW1234"},
        )
        user_token = user.data["access"]
        response = self.client.get(
            "/api/v1/booking", HTTP_AUTHORIZATION=f"JWT {user_token}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_post_booking(self):
        ...
        # normal data
        response = self.client.post(
            "/api/v1/booking", data=booking_data, HTTP_AUTHORIZATION=f"JWT {user_token}"
        )
...
class MyBookingsViewTest(TestCase):
    def setUp(self) -> None:
        ...
        first_user = User.objects.create_user(
            username="first_test_api_user", password="QazxsW1234"
        )
        second_user = User.objects.create_user(
            username="second_test_api_user", password="QazxsW1234"
        )
    ...
    def test_get_my_booking(self):
        first_agent = self.client.post(
            "/auth/jwt/create",
            data={"username": "first_test_api_user", "password": "QazxsW1234"},
        )
        first_agent_token = first_agent.data["access"]

        second_agent = self.client.post(
            "/auth/jwt/create",
            data={"username": "second_test_api_user", "password": "QazxsW1234"},
        )
        ...
        response = self.client.post(
            "/api/v1/booking",
            data=first_booking_data,
            HTTP_AUTHORIZATION=f"JWT {first_agent_token}",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        response = self.client.post(
            "/api/v1/booking",
            data=second_booking_data,
            HTTP_AUTHORIZATION=f"JWT {second_agent_token}",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        response = self.client.get(
            "/api/v1/my_bookings/", HTTP_AUTHORIZATION=f"JWT {second_agent_token}"
        )
