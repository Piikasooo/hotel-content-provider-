...
def now_currency():
    ...
    date_name_file = f"currency/{str(datetime.date.today())}.csv"
...
def get_currency(date_today, currency_name):
    date_name_need = f"openexchange/currency/{str(date_today)}.csv"
...
def start_server():
    while True:
        date_name = f"currency/{str(datetime.date.today())}.csv"
