...
urlpatterns = [
    ...
    path(
        "coefficient/<slug:slug>/", CreateCoefficientView.as_view(), name="coefficient"
    ),
    path(
        "rooms/<slug:slug>/<int:room_number>/",
    ...
    path(
        "amenity/<slug:slug>/<str:amenity_name>/",
        AmenityUpdate.as_view(),
        name="amenity_update",
    ),
    path(
        "room_type/<slug:slug>/<str:room_type_name>/",
        TypeRoomUpdate.as_view(),
        name="type_room_update",
    ),
    path(
        "coefficient_update/<slug:slug>/<int:id>/",
        CoefficientUpdate.as_view(),
        name="coef_update",
    ),
    path(
        "room/<slug:slug>/<int:room_number>/",
        RateUpdateView.as_view(),
        name="rate_update",
    ),
    path(
        "add_hotel_image/<slug:slug>/", AddHotelImage.as_view(), name="add_hotel_image"
    ),
]
