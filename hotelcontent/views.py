    ...
    def post(self, request, slug):
        ...
        try:
            ...
            room = Rooms(
                ...
                room_type=room_type,
                hotel=hotel,
                room_rate_price=room_rate_price,
            )
            room.save()
        except ValueError:
...
class RoomsView(View):
    def get(self, request, slug):
        ...
        context = {"user": user, "rooms": rooms, "hotel": hotel}
...
class RateUpdateView(View):
    ...
    def post(self, request, slug, room_number):
        ...
        if request.POST.get("update"):
            ...
            room_number = request.POST.get("room_number")
            allrooms = Rooms.objects.exclude(room_number=room.room_number).filter(
                hotel=hotel
            )
            print(allrooms)
            for roomhotel in allrooms:
            ...
            room_type_object = RoomTypes.objects.get(
                room_type_name=room_type, hotel=hotel
            )
            ...
            bookings_room = Bookings.objects.filter(
                hotels=hotel, room=room, booking_stat=True
            )
