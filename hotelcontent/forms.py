...
class AddHotelForm(forms.ModelForm):
    ...
    hotel_lat = forms.DecimalField(
        max_digits=9, decimal_places=6, required=True, label="Latitude"
    )
    ...
    def clean(self):
        ...
        if Hotel.objects.filter(hotel_name=hotel_name).exists():
            raise forms.ValidationError(
                f"Hotel with name {hotel_name} is already in use!"
            )
        elif Hotel.objects.filter(hotel_long=hotel_long, hotel_lat=hotel_lat).exists():
            raise forms.ValidationError(
                f"Hotel with coordinates {hotel_lat}, {hotel_long} is already in use!"
            )
        elif Hotel.objects.filter(hotel_email=hotel_email).exists():
            raise forms.ValidationError(
                f"Hotel with email {hotel_email} is already in use!"
            )
        elif Hotel.objects.filter(hotel_url=hotel_url).exists():
            raise forms.ValidationError(
                f"Hotel with site {hotel_url} is already in use!"
            )
...
class AddRoomTypeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if RoomTypes.objects.filter(
            room_type_name=room_type_name, hotel=hotel
        ).exists():
            raise forms.ValidationError(
                f"Room type with a name {room_type_name} already exists!"
            )
