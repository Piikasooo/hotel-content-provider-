...
class TestCreateCoefficientForm(TestCase):
    ...
    def test_create_coefficient_no_valid_form(self):
        ...
        self.assertEquals(
            errors_filed["end_date"], "* Конечная дата выбрана некоректно"
        )
        self.assertEquals(errors_filed["__all__"], "* Коефицент введен неверно")
...
class TestLoginForm(TestCase):
    ...
    def test_login_form_worng_password(self):
        ...
        for field in form.errors:
            errors_filed[field] = form.errors[field].as_text()
        self.assertEquals(errors_filed["__all__"], f"* Неверный пароль")
...
class TestRegistrationForm(TestCase):
    ...
    def test_registration_form_no_valid_data(self):
        ...
        for field in form.errors:
            errors_filed[field] = form.errors[field].as_text()
        self.assertEquals(
            errors_filed["username"], f'* Имя {registration_data["username"]} занято'
        )
        self.assertEquals(
            errors_filed["email"], "* Данный почтовый адресс уже зарегестрирован"
        )
        self.assertEquals(errors_filed["__all__"], "* Пароли не совпадают")
...
class TestAddHotelForm(TestCase):
    ...
    def test_add_hotel_form_temp(self):
        ...
        for field in form.errors:
            errors_filed[field] = form.errors[field].as_text()
        self.assertEquals(
            errors_filed["hotel_long"],
            "* Ensure that there are no more than 9 digits in total.",
        )
        self.assertEquals(
            errors_filed["hotel_lat"],
            "* Ensure that there are no more than 9 digits in total.",
        )
        self.assertEquals(errors_filed["hotel_email"], "* Enter a valid email address.")
        self.assertEquals(errors_filed["hotel_url"], "* Enter a valid URL.")
        self.assertFalse(form.is_valid())
...
class TestAddRoomTypeRoom(TestCase):
    ...
    def test_add_room_type_no_data(self):
        ...
        for field in form.errors:
            errors_filed[field] = form.errors[field].as_text()
        self.assertEquals(errors_filed["room_type_name"], "* This field is required.")
        self.assertEquals(
            errors_filed["room_type_description"], "* This field is required."
        )
        self.assertEquals(errors_filed["room_type_price"], "* This field is required.")
