...
class TestView(TestCase):
    ...
    def test_add_room_type_GET(self):
        self.assert_GET_method(
            "add_room_type", "add_room_type.html", ["setup_hotelHotel"]
        )

    def test_add_room_type_POST(self):
        hotel = Hotel.objects.get(hotel_name="setup_hotel")
        response = self.client.post(
            reverse("add_room_type", args=["setup_hotelHotel"]),
            {
        ...
        self.assertEquals(response.status_code, 302)
        self.assertTrue(
            RoomTypes.objects.filter(room_type_name="test_room_type").exists()
        )
    ...
    def test_amenity_POST(self):
        hotel = Hotel.objects.get(hotel_name="setup_hotel")
        response = self.client.post(
            reverse("amenity", args=["setup_hotelHotel"]),
            {
    ...
    def test_add_room_POST(self):
        ...
        response = self.client.post(
            reverse("add_room", args=["setup_hotelHotel"]),
            {
                "dropdown": room_type,
                "room_number": 1,
                "hotel": hotel,
